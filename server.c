...
void client_request_handler(int clientFD, struct sockaddr_in client){
    ...
    {
        send_client_msg(clientFD,msgInputPassword331,strlen(msgInputPassword331));
        while(1)
        {
    ...
    else
    {
        ...
        {
            printf("client %d logged in, username:%s password:%s\n",getpid(),username,password);
            send_client_msg(clientFD, msgWelcome230, strlen(msgWelcome230));
            login_flag = 2;
        }
        else
        {
            send_client_msg(clientFD, msgNotLoggedIn530, strlen(msgNotLoggedIn530));
            login_flag = 0;
        }
    }
    ...
    while (receive_client_message(clientFD)) {
        ...
        else if(login_flag > 0 && strncmp("TYPE", msg_buf, 4)== 0)
        {
            ...
            {
                send_client_msg(clientFD, msgNotImplemented202, strlen(msgNotImplemented202));
            }
        }
...
void Port(int clientFD){
    ...
    send_client_msg(clientFD, msgPortSuc200, strlen(msgPortSuc200));
}
...
void Retr(int clientFD){
    ...
    if(connect_mode == 0)   //PASV mode
    {
        ...
        {
            send_client_msg(clientFD, msgOpenConnectionF425, strlen(msgOpenConnectionF425));
            close(connect_data_sock);
            close(ftp_data_sock);
            close(readFD);
            return;
    ...
    ftp_data_sock = -1;
    connect_data_sock = -1;
    send_client_msg(clientFD, msgTranComp226, strlen(msgTranComp226));
...
void Stor(int clientFD){
    ...
    if(writeFD < 0)
    {
        send_client_msg(clientFD,  msgWriteFailed451, strlen(msgWriteFailed451));
        close(connect_data_sock);
        close(ftp_data_sock);
        return;
    }
